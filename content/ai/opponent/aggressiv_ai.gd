...
func pick_target():
	var player_planets = get_player_planets()
	player_planets.sort_custom(sort_unit_count.bind(true))
	var self_planets = get_hostile_planet()
	...
	elif not get_neutral_planets().is_empty():
		launch_attack(self_planets[0], get_neutral_planets().pick_random())
	else:
